...
from carla_sgg.sgg import SGG, IgnoreWaypointPickler
from carla_sgg.sgg_abstractor import process_to_junction_rsv
from carla_sgg.utils import plot_graph
...
SENSORS = [
    [
        'CameraTest',
        {
            ...
            'image_size_x': 1080, 'image_size_y': 720, 'fov': 100,
...
RECORDER_INFO = [
    {
        'folder': "leaderboard2/ScenarioLogs/OppositeVehicleTakingPriority",
        'name': 'OppositeVehicleTakingPriority',
        'start_time': 0,
        'duration': 0
    }
]
    ...
    for sensor_id, sensor_bp in sensors:
        ...
        if 'imu' in sensor_bp:
            ...
            with open(sensor_endpoint, 'w') as data_file:
                ...
                data_file.write(data_txt)
    os.makedirs(f'{endpoint}/rsv/', exist_ok=True)
...
def main():
    ...
    argparser.add_argument('--destination_folder', default="database", type=str, help='Destination folder for collected data.')
    argparser.add_argument('--record', type=Path, help='Path to the CARLA record folder.')
    args = argparser.parse_args()
    print(__doc__)
    print("Starting the sensor data capture script...")
    ...
    if args.record is not None:
        ...
        RECORDER_INFO = [{'folder': str(args.record), 'name': args.record.name, 'start_time': 0, 'duration': 0}]
    ...
    try:
        ...
        for recorder_info in RECORDER_INFO:
            ...
            if recorder_path_list:
                ...
                raise Exception("Recorder file not found")
            ...
            sgg = SGG(client, get_ego_id(recorder_str))
            ...
            while True:
                ...
                sg_frame = frame - start_frame
                sg = sgg.generate_graph_for_frame(sg_frame)
                rsv = process_to_junction_rsv(sg)
                plot_graph(rsv, f'{endpoint}/rsv/{sg_frame}.png')
                with open(f'{endpoint}/rsv/{sg_frame}.pkl', 'wb') as f:
                    IgnoreWaypointPickler(f).dump(rsv)
